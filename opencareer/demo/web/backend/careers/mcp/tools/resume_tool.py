...
def resume_skill(
    ...
    try:
        ...
        json_path = str(output_folder / f"{safe_name}.json") if not output_json_path else output_json_path
        saved["json"] = _write_json(json_path, resume)

        pdf_path = str(output_folder / f"{safe_name}.pdf") if not output_pdf_path else output_pdf_path
