#!/usr/bin/env python3
# 保存路径: E:\OneDrive\Desktop\职场情感伙伴技术部分\resume-cn-career\mcp_resume_cn_career.py
# Claude Desktop 配置示例（claude_desktop_config.json）:
# {
#   "mcpServers": {
#     "resume-cn-career": {
#       "command": "C:/Users/123/AppData/Local/Programs/Python/Python314/python.exe",
#       "args": ["E:/OneDrive/Desktop/职场情感伙伴技术部分/resume-cn-career/mcp_resume_cn_career.py"],
#       "cwd": "E:/OneDrive/Desktop/职场情感伙伴技术部分/resume-cn-career"
#     }
#   }
# }
...
PDF_SCRIPT = ROOT / "scripts" / "generate_resume_pdf.py"

mcp = FastMCP("resume-career")
...
def _export_pdf(resume: dict[str, Any], output_pdf_path: str) -> str:
    if not PDF_SCRIPT.exists():
        raise ResumeSkillError(f"未找到 PDF 导出脚本：{PDF_SCRIPT}")
    ...
    try:
        ...
        raise ResumeSkillError(f"PDF 导出失败：{message}") from exc
    finally:
        try:
...
@mcp.tool(description="封装中文简历生成、优化、ATS 检查与 PDF 导出的工作流。根据输入的个人信息、目标岗位和 JD 生成结构化简历草稿，并可选择导出 PDF 或执行静态检查。")
def resume_skill(
    action: Annotated[
    ...
    output_json_path: Annotated[str, "输出 JSON 路径，留空则不落盘"] = "",
    output_pdf_path: Annotated[str, "输出 PDF 路径，留空则不导出"] = "",
) -> dict[str, Any]:
    """
    基于 resume-cn-career Skill 的 MCP 工具。
    ...
    try:
        ...
        if action == "export_pdf" and not _clean_text(output_pdf_path):
        ...
        saved: dict[str, str] = {}
        if _clean_text(output_json_path):
            saved["json"] = _write_json(output_json_path, resume)

        if action == "export_pdf":
            saved["pdf"] = _export_pdf(resume, output_pdf_path)
        ...
        return result
    except ResumeSkillError as exc:
        return {
            "ok": False,
            "error": str(exc),
            "action": action,
