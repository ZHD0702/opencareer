...
使用外面的 opencareer 包中的 CareerAgent。
"""
...
# 添加项目根目录到 Python 路径，以便导入外面的 opencareer 包
_project_root = Path(__file__).resolve().parent.parent.parent  # F:\opencareer\web\backend\agent_factory.py -> F:\opencareer
...
class AgentFactory:
    ...
    def create_agent(self, agent_type: str = "simple", **kwargs):
        ...
        if agent_type == "simple":
            ...
            logger.info("Creating CareerAgent from opencareer package")
            try:
                # 从外面的 opencareer 包导入完整的 CareerAgent
